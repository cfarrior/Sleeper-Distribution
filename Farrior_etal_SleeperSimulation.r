...
######################################
main_cohorts = function(runV=c(filestem="default",PA=1000,deltaT=1,Fnot=0.05,dnot=0.02,G.c=6.05,G.u=0.534,mu.c=0.0097,mu.u=0.025,mu=0.0097,cutT=400,landscape_m2=125000,censuscutT=5),tag=1,newplot=FALSE,plotting=TRUE){
...
#maxT: the runtime in years of the run
#cutT: the time between recording snapshots of the forest - these used to build a landscape of independent forests
#newplot: whether to create a new plotting window
#plotting: whether to plot the size distribution during the simulation, slows the run speed considerably
#maxT: the run time in years
#wipeout: logical, TRUE if the stand is resent to starting conditions at uniform yearly probability (run at the timestep level) of mu
######################################
	filestem = runV[names(runV)=="filestem"][[1]]

	if(tag==1) write.table(cbind(date(),t(runV)),paste(mainfolder,"main_cohorts_LOG.txt",sep=""),append=TRUE,sep="\t",col.names=filestem=="start",row.names=FALSE)
	...
	mu.u_exp = as.numeric(runV[names(runV)=="mu.u_exp"][[1]])
	...
	censuscutT = as.numeric(runV[names(runV)=="censuscutT"][[1]])
	...
	for(t in seq(0,maxT,by=deltaT)){
		...
		CA = sum(phi*data[,1]^theta*data[,2])
		if(CA<=PA){ data[,3]=1; data[,4]=0}
		if(CA>PA) data = CCassign(data,PA,deltaT)
		
		#Step 5: Record and plot
		if(floor(t/cutT)==t/cutT){
			if(dim(data)[1]>0){
				dstar = find_dstar(get_dV(data),PA)
				CAi = sum(data[data[,1]>10,1]^theta*phi*data[data[,1]>10,2])/PA
				write.table(cbind(data,t,t_age,dstar,CAi),paste(filestem,tag,".txt",sep=""),sep="\t",col.names=t==cutT,row.names=FALSE,append=t!=cutT)
			}
		}
...
######################################
SizeDistPlot = function(data,logby=.14,win=3,sizem2=NaN,main="",census=5,ploton=TRUE,newplot=TRUE,col=1,xaxt="s",yaxt="s",ymin=1e-4,xmax=3500,binV=NaN,justbdata=FALSE,powerfit=allfit){
#function used in the main_cohorts, main_individuals_forcensus simulations to plot the size distribution during the simulation run
#this function slows down the speed of the simulation quite a bit.
#data: either a vector of diameters of individuals in the community OR a matrix with the first column the diameter and the second column the number of individuals (This flexibility is new and needs testing. Before data could only be the matrix. This might make some old function unnecessary. 150408 !?)
